...
save. Reload Jellyfin Web and the palette is there. Nothing goes in the Custom CSS
field, and nothing needs a restart.

The plugin adds the compiled bundle to Jellyfin Web's `index.html` response as
Jellyfin serves it, alongside the pre-paint layer and the browser helper, reading
the configured palette per request. Nothing is written to the web folder, so this
works on a server whose Jellyfin Web directory the service account cannot write —
a container running as a non-root user, a read-only image layer — and it survives a
Jellyfin Web upgrade replacing the file.

The bundle is added at the end of `<body>`, the only place it outranks the palette
Jellyfin Web installs at runtime, so it also outranks the Custom CSS field:
overrides pasted there need `!important` while the plugin is serving the theme.
...
Turn **Write the assets into index.html instead of serving them** on only where
something other than Jellyfin serves the web folder — a reverse proxy pointed
straight at it, for one. The plugin then edits the file on disk at startup and on
every palette change, which needs the service account to have write access to it
and is undone by every Jellyfin Web upgrade until the next restart. With it off,
the plugin also clears any blocks releases 1.1.24 and 1.1.25 left in that file.
...
For an installation without an external stylesheet request, download a compiled CSS file from the [v1.1.26 release](https://github.com/chadouming/jellyfin-theme-witzi/releases/tag/v1.1.26) and paste its full contents into the Custom CSS field. The compiled files are standalone and contain their SVG pattern as an embedded data URI.

For a version-pinned CDN import, use `https://cdn.jsdelivr.net/gh/chadouming/jellyfin-theme-witzi@v1.1.26/dist/witzi-mocha.css` and change the filename for another palette. Version-pinned links only change when you deliberately select a newer release.
...
reaches every client and the dashboard, because it is part of the page as it is
served rather than something the application fetches and injects after it starts.
